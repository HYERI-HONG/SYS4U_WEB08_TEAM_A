(function(global, doc) {
	...
	instagram.Post.prototype.initialize = function(vo) {
		this.vo = vo;
		var that = this;
		
		this.memberProfile = "";
		if (vo.memberImage != null) {
			this.memberProfile = _actionUrl + vo.memberImage.path + "/" + vo.memberImage.name + "." + vo.memberImage.extension;
		}
		
		this.postImage = "";
		if (vo.postImage != null) {
			this.postImage = _actionUrl + vo.postImage.path + "/" + vo.postImage.name + "." + vo.postImage.extension;
		}
		
		this.postLikes = "";
		vo.postLikes.length == 1 ? this.postLikes = vo.postLikes[0].likeUser + "님이 좋아합니다."
				: vo.postLikes.length > 1 ? this.postLikes = vo.postLikes[0].likeUser + "님외 " + String(vo.postLikes.length - 1) + "명이 좋아합니다."
						: this.postLikes = "";
				
		this.content = vo.content;
		if (vo.compressedContent != null) {
			const moreContent = `<span class="moreContent" id="moreContent-${vo.no}">더보기</span>`;
			this.content = vo.compressedContent + moreContent;
		}
		...
		this.postReplies = "";
		this.postUserId = "";
		if (vo.postReplies.length == 1) {
			this.postUserId = vo.postReplies[0].userId;
			this.postReplies = vo.postReplies[0].content;
		} else if (vo.postReplies.length > 1) {
			this.postUserId = vo.postReplies[0].userId;
			this.postReplies = vo.postReplies[0].content + `<div id="postDetail-${vo.no}">더보기</span>`;
		}
